...
export default class Car
{
    ...
    setThemeMaterials()
    {
        const bodyMaterial = new THREE.MeshBasicMaterial({ color: 0xffe8c8 })
        const accentMaterial = new THREE.MeshBasicMaterial({ color: 0x3f93c5 })
        const darkMaterial = new THREE.MeshBasicMaterial({ color: 0x243b53 })
        ...
        this.chassis.object.traverse((child) =>
        {
            ...
            if(name.includes('window') || name.includes('glass') || materialName.includes('black'))
            {
                child.material = darkMaterial
            }
            else if(name.includes('light') || name.includes('line') || materialName.includes('blue'))
            {
                child.material = accentMaterial
            }
            else if(materialName.includes('red') || name.includes('body') || name.includes('chassis') || !name)
            {
    ...
    setTransformControls()
    {
        ...
        this.transformControls.helper = this.transformControls.getHelper ? this.transformControls.getHelper() : this.transformControls
        this.transformControls.helper.visible = this.transformControls.enabled
        ...
        this.container.add(this.transformControls.helper)
